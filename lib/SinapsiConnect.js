    ...
    try {
      ...
      for (const sensor of this.sensors) {
        ...
        if (sensor.id === "alarm_generic") {
          ...
          this.eventDate = (result.value === -1 || result.value === 65535 || result.value === SinapsiConnect.DISCONNECT_ALARM_INACTIVE) ? -1 : result.value;
      ...
      if (wasSuccessful) {
        ...
        if (this.eventDate !== undefined && this.remainingDisconnectionTime !== undefined) {
          ...
          if (this.eventDate !== -1) {
            // Use remainingDisconnectionTime directly - it's already the countdown from the Modbus sensor
            const secondsRemaining = Math.max(0, this.remainingDisconnectionTime);
