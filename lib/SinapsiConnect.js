...
class SinapsiConnect {
   ...
   * Emits 'disconnectionWarning', 'firstDisconnectionWarning' or 'stopWarning' events when appropriate.
  ...
  readData() {
    ...
    return Promise.all(promises).then(() => {
      if (this.eventDate !== undefined && this.remainingDisconnectionTime !== undefined) {
        ...
        if (this.eventDate !== -1) {
          this.homey.emit('disconnectionWarning', this.remainingDisconnectionTime);
          if (!this.warningStarted) {
            this.warningStarted = true;
            this.homey.emit('firstDisconnectionWarning', this.remainingDisconnectionTime);
          }
        } else {
          if (this.warningStarted) {
            this.warningStarted = false;
            this.homey.emit('stopWarning');
          }
        }
      } else {
        this.warningStarted = false;
