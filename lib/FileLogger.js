...
class FileLogger {
  ...
  async _rotateIfNeeded() {
    try {
      ...
      // Rotate backups: rename alfa_debug.1.log → alfa_debug.2.log, etc.
      for (let i = this.maxBackups - 1; i >= 1; i--) {
        const oldFile = `/userdata/alfa_debug.${i}.log`;
        const newFile = `/userdata/alfa_debug.${i + 1}.log`;
      ...
      // Rename current file: alfa_debug.log → alfa_debug.1.log
      fs.renameSync(this.logFile, '/userdata/alfa_debug.1.log');
  ...
  _logMemoryUsage() {
    try {
      ...
      // Enhanced error handling for ENOENT and other memory check failures
      if (error.code === 'ENOENT') {
        this.warn('MEMORY', 'Memory check temporarily unavailable (ENOENT) - skipping this cycle');
      } else {
  ...
  async deleteLog() {
    try {
      ...
      for (let i = 1; i <= this.maxBackups; i++) {
        const backupFile = `/userdata/alfa_debug.${i}.log`;
